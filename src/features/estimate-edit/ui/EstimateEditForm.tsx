...
import './EstimateEditForm.css'

export function EstimateEditForm() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const estimate = id ? estimateStore.getById(id) : null
  ...
  const handleSave = () => {
    ...
    estimateStore.update(estimate.id, {
      name: name.trim(),
      items,
      total,
      currency,
...
function EstimateItemRow({ item, onUpdate, onRemove }: EstimateItemRowProps) {
  ...
  useEffect(() => {
    // Синхронизируем только при изменении item, но не перезаписываем если пользователь редактирует
    if (item.serviceName && !serviceSearch) {
